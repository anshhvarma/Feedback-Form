...
public class DButils extends loginInfo{
    public static ArrayList<List<String>>  Data() throws Exception{
        ...
        // String Header[] = {"ERP","Name","Password"};
        // data.add(Arrays.asList(Header));
        
        
        BufferedReader br = new BufferedReader(new FileReader(FilePath+"\\DATA.csv"));  //FileNotFound
        String line="";
    ...
    // public static int rowit(ArrayList<List<String>> data,String ERP,String Password){
    //     int row=-1;
    //     for(int i=1;i<data.size();i++){
    //         if(ERP.compareTo((data.get(i).get(0)))==0 && (data.get(i).get(2)).compareTo(Password)==0){
    //             row=i;
    //         }
    //     }
    //     return row;
    ...
    public static void changeScene(ActionEvent event,String title,String fxmlFile, String ERP, String Password,int check){
        Parent root = null;
        if(ERP != null && Password!= null && check==0){  //if the fields are not empty
            try{
            ...
            try{
                ...
                if(fxmlFile == "start.fxml"){
                    ...
                    start.setUserInformation(ERP,Password);
                }
                else if(fxmlFile == "form.fxml"){
                    FormController form = loader.getController();
                    form.setUserInformation(ERP, Password);
                }
            }catch(Exception e){
                System.out.println(e);
            }
    ...
    public static void login(ActionEvent event, String ERP, String Password,ArrayList<List<String>>  data){
        for(int i=1;i<data.size();i++){
            if(ERP.compareTo((data.get(i).get(0)))==0 && (data.get(i).get(2)).compareTo(Password)==0){
                loginInfo.ERP=ERP;
                ...
                if(data.get(row).get(5).compareTo("0")==0){  //checks whether already submitted or not
                    changeScene(event,"Start Page", "start.fxml", ERP, Password,1);
                }else{
                    Alert alert = new Alert(Alert.AlertType.ERROR);
    ...
    public static void submitted() throws Exception{
        ...
        String FilePath = System.getProperty("user.dir");
        BufferedWriter bw=new BufferedWriter(new FileWriter(FilePath+"\\DATA.csv"));
        int row= DButils.row;
        for(int i=0;i<data.size();i++){
            for(int j=0;j<data.get(0).size();j++){
                if(i==row && j==5){
